...
def test_get_tool_turn_metrics_returns_list():
    metrics = eval_cases.get_tool_turn_metrics()
    assert isinstance(metrics, list)


def test_get_tool_turn_metrics_contains_faithfulness():
    metrics = eval_cases.get_tool_turn_metrics()
    assert "ragas:faithfulness" in metrics


def test_get_tool_turn_metrics_is_cached():
...
def test_get_metric_thresholds_returns_dict():
    thresholds = eval_cases.get_metric_thresholds()
    assert isinstance(thresholds, dict)


def test_get_metric_thresholds_faithfulness_threshold():
    thresholds = eval_cases.get_metric_thresholds()
    assert thresholds.get("ragas:faithfulness") == 0.7


def test_get_metric_thresholds_answer_correctness():
    thresholds = eval_cases.get_metric_thresholds()
    assert thresholds.get("custom:answer_correctness") == 0.75


def test_get_metric_thresholds_is_cached():
...
def test_load_cases_returns_empty_for_missing_file(tmp_path, caplog):
...
def test_load_cases_from_file(tmp_path):
    data = [{"conversation_group_id": "c1", "tag": "non_hitl", "turns": [
        {"turn_id": "t1", "query": "Q", "expected_response": "A",
...
def test_load_cases_normalizes_expected_keywords(tmp_path):
    data = [{"conversation_group_id": "c1", "tag": "non_hitl", "turns": [
        {"turn_id": "t1", "query": "Q", "expected_response": "A",
         "turn_metrics": [], "expected_keywords": "22.9, Normal"}
    ]}]
    f = tmp_path / "eval_cases.yaml"
    f.write_text(yaml.dump(data))
    cases = eval_cases.load_cases(f)
...
def test_load_cases_accepts_directory(tmp_path):
...
def test_filter_cases_by_tag_match(tmp_path):
    data = [
...
def test_filter_cases_by_tag_no_match(tmp_path):
    data = [{"conversation_group_id": "c1", "tag": "tool_use", "turns": []}]
    f = tmp_path / "eval_cases.yaml"
    f.write_text(yaml.dump(data))
...
def test_filter_cases_by_tag_empty(tmp_path):
    f = tmp_path / "eval_cases.yaml"
